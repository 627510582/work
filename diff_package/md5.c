...
char **argv;
{
	...
	{
		if (CONFIG_TIMEOUT < time() - ts)
		{
			goto err;
		}

		z_md5_append(&state, (const md5_byte_t *)buf, nbytes);
	}

	z_md5_finish(&state, hash);
	...
	hash_text = z_malloc(hash_text, sz);

	/*转换成32位hash*/
	for (i = 0; i < MD5_DIGEST_SIZE; i++)
	{
		z_snprintf(&hash_text[i << 1], sz - (i << 1), "%02x", hash[i]);
	}
	/*转换成16位hash*/
	for (i = 4,j = 0; i < 12; i++,j++) {  
		z_snprintf(&hash_text[j << 1], sz - (j << 1), "%02x", hash[i]);
	}  
